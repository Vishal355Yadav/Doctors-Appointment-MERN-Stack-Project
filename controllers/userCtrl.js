...
const user = await userModel.findById({ _id: req.body.userId});
        ...
        console.log(error)
        res.status(500).send({ 
            message:'auth user', success :false,error
        })
    }
};
...
const applyDoctorController=async(req,res)=>{
    try{
        ...
        console.log(error);
        res.status(500).send({
            success:false,error,message:"Error while Applying for Doctor"
        })
    }
};
...
const getAllNotificationController=async(req,res)=>{
    try{
        ...
        message:'all notification marked as read',data: updatedUser});
...
const deleteAllNotificationController =async(req,res)=>{
    try{
        ...
        console.log(error)
        res.status(500).send({
            success:false, message:'unable to delete all notification',
            error
        })
    }
};
...
const getAllDoctorsController= async(req,res)=>{
    try{
        ...
        res.status(500).send({
            success:false,
            error,
            message:'Wrror while Fetching Doctors'
        })
    }
};
...
const bookAppointmentController=async(req,res) =>{
    try{
        ...
        user.notification.push({type:'New-appointment-request',
...
await user.save();
    res.status(200).send({
        ...
        error,
        message: 'Error while Booking appointemnt'})
        
    }
};

// booking avalability controller
